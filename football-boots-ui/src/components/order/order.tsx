...
export const Order = () => {
  ...
  async function order() {
    const res = await axios.put("http://localhost:10000/users/order", { email: localStorage.getItem("userEmail"), price: localStorage.getItem("totalPrice") },
      {
        headers: {
          authorization: 'Bearer ' + localStorage.getItem('token') as string
        }
        ...
        localStorage.setItem("messageError", error.response.data)
        window.location.replace('http://localhost:3000/loading')
      })
  }
