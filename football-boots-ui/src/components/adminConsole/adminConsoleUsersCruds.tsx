...
export const AdminConsoleUsers = () => {
  ...
  async function getLogs() {
    const res = await axios.get("http://localhost:10000/users/logs", {
      headers: {
        authorization: 'Bearer ' + localStorage.getItem('token') as string
      }
    }).then(function (res) {
  ...
  async function getUsersOnline() {
    const res = await axios.get("http://localhost:10000/users/logs/online", {
      headers: {
        authorization: 'Bearer ' + localStorage.getItem('token') as string
      }
    }).then(function (res) {
  ...
  async function searchLogs() {
    const res = await axios.get("http://localhost:10000/users/logs/user/" + searchEmail, {
      headers: {
        authorization: 'Bearer ' + localStorage.getItem('token') as string
      }
    }).then(function (res) {
  ...
  return (
    <Box sx={{ flexGrow: 1 }}>
      ...
      <Typography sx={{ position: 'absolute', marginTop: '508px', marginLeft: '1530px' }}>{onlineUsers}</Typography>
      <IconButton color="success" sx={{ position: 'absolute', marginTop: '500px', marginLeft: '1550px' }}>
        <CircleIcon />
      </IconButton>
      <IconButton color="primary" sx={{ position: 'absolute', marginTop: '500px', marginLeft: '1260px' }}
        onClick={handleSearch}>
        <SearchIcon />
      </IconButton>
      <IconButton color="primary" sx={{ position: 'absolute', marginTop: '500px', marginLeft: '1300px' }}
        onClick={handleRefresh}>
        <RefreshIcon />
      </IconButton>
      <TextField size='small' sx={{ position: 'absolute', marginTop: '500px', marginLeft: '950px', width: '300px' }} onChange={event => setSearchEmail(event.target.value as string)}></TextField>
      <Box sx={{ border: '2px black solid', width: '700px', position: 'absolute', marginTop: '550px', marginLeft: '900px' }}>
        <TableContainer component={Box}>
          <Table aria-label="simple table">
            <TableHead>
              <TableRow>
                <TableCell><Typography sx={{ fontSize: '20px' }}>Operation</Typography></TableCell>
                <TableCell align="right"> <Typography sx={{ fontSize: '20px' }}>User</Typography></TableCell>
                <TableCell align="right"><Typography sx={{ fontSize: '20px' }}>Timestamp</Typography></TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              {logs.map((row: any) => (
                ...
                >
                  <TableCell component="th" scope="row">
                    <Typography sx={{ fontSize: '20px' }}>{row.operation}</Typography>
                  </TableCell>
                  <TableCell align="right"><Typography sx={{ fontSize: '20px' }}>{row.user}</Typography></TableCell>
                  <TableCell align="right"><Typography sx={{ fontSize: '20px' }}>{row.timestamp}</Typography></TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
